...
describe('#compileMethodsToDynamodb()', () => {
  ...
  describe('#get method', () => {
    it('should create corresponding resources when hashkey is given with a path parameter', () => {
      ...
      const intResponseTemplate = `#set($item = $input.path(\'$.Item\')){#foreach($key in $item.keySet())#set ($value = $item.get($key))#foreach( $type in $value.keySet())"$key":"$util.escapeJavaScript($value.get($type)).replaceAll("\\\\'","'")"#if($foreach.hasNext()),#end#end#if($foreach.hasNext()),#end#end}`
      testGetItem(
        {
          hashKey: { pathParam: 'id', attributeType: 'S' },
          path: '/dynamodb/{id}',
