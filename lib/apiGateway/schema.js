...
const authorizerId = Joi.alternatives().try(
  ...
  Joi.object()
    .keys({
      Ref: Joi.string(),
      'Fn::ImportValue': Joi.string(),
      'Fn::GetAtt': Joi.array()
        .length(2)
        .ordered(Joi.string().required(), Joi.string().required())
    })
    .xor('Ref', 'Fn::ImportValue', 'Fn::GetAtt')
    .error(
      customErrorBuilder(
        'object.xor',
        'As object, you must either specify "Ref", "Fn::ImportValue" or "Fn::GetAtt" as key'
      )
    )
)
